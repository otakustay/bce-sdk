...
export class BosClient {
     ...
     * @see https://cloud.baidu.com/doc/BOS/s/Ikc5nv3wc
     */
    async putObjectJson(bucketName: string, key: string, body: JsonObjectBody, options?: PutObjectOptions) {
        return this.withObject(bucketName, key).putJson(body, options);
    }
     ...
     */
    async uploadObjectByMultipart(
        bucketName: string,
        key: string,
        file: string,
        options: UploadFileByMultipartOptions,
    ) {
        return this.withObject(bucketName, key).uploadFileByMultipart(file, options);
    }
